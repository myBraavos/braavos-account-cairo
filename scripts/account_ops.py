...
async def declare(client: FullNodeClient, account: Account, casm_path: str, sierra_path: str, retries = 5):
    ...
    casm_chash = compute_casm_class_hash(
        CasmClassSchema().loads(casm_content))
    sierra_chash = compute_sierra_class_hash(
        SierraCompiledContractSchema().loads(sierra_content,
                                             unknown="exclude"))
...
def create_stark_signer(stark_privk: int, mock_est_fee=False):
    ...
    def sign_txn(txn: AccountTransaction):
        if not mock_est_fee or txn.version < 2**128:
            return [
                STARK_SIGNER_TYPE, *message_signature(
...
def create_secp256r1_signer(ecc_key: ec.EllipticCurvePrivateKey,
    ...
    def sign_txn(txn: AccountTransaction):
        ...
        hash_bytes = txn_hash.to_bytes((txn_hash.bit_length() + 7) // 8,
                                       byteorder="big",
                                       signed=False)
...
async def account_deployer(
    ...
    signed_account_depl = await deployer_account.sign_deploy_account_transaction(
        class_hash=base_account_chash,
...
def compute_myswap_cl_to_usdc_price(target_rate_usdc_for_token, token_decimal, is_usdc_token0):
    # USDC decimals == 6
    ...
    output_rate = (target_rate_usdc_for_token * (10**6 / 10**token_decimal)) ** (-1 if is_usdc_token0 else 1)  * (2 ** 96)
...
async def execute_calls(account: Account,
                        ...
                        max_fee=None):
    if max_fee is None:
        invoke_txn = await account.sign_invoke_transaction(
