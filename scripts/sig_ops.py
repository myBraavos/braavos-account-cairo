...
def check_secp256r1_sig(txn_hash):
    chain_txn_raw = requests.get(f"https://alpha-mainnet.starknet.io/feeder_gateway/get_transaction?transactionHash={hex(txn_hash)}").json()
    ...
    signer_pub_key_raw = requests.post(f"https://alpha-mainnet.starknet.io/feeder_gateway/call_contract?blockNumber={block_num}", json={
        "signature": [],
    ...
    public_key.verify(encoded_sig, message_hash_bytes, ec.ECDSA(Prehashed(hashes.SHAKE256(len(message_hash_bytes)))))
