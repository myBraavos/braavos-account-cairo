...
class TypedDataR1:
    ...
    def __init__(self, types, domain, version):
        self.types = {
            "StarknetDomain": [
                {
                    "name": "name",
    ...
    def get_hash(self, message, account_address, message_name):
        message_struct = [
            encode_shortstring("StarkNet Message"),
            self.struct_hash("StarknetDomain", self.domain),
            account_address,
            self.struct_hash(message_name, message),
        ]
    ...
    def _encode_data(self, type_name: str, data: dict) -> List[int]:
        values = []
        for param in self.types[type_name]:
            encoded_value = self._encode_value(param["type"],
                                               data[param["name"]])
    ...
    def _encode_value(self, type_name: str, value: Union[int, str, dict,
                                                         list]) -> int:
